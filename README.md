...
- **Shape recognition** turns completed freehand strokes that clearly resemble a circle, ellipse, rectangle, diamond, triangle, straight line or arrow into crisp, perfect shapes at the exact position where they were drawn. Strokes may be wobbly or slightly overrun (a tail that passes the closing point is trimmed automatically). Conversion happens about 1.5 seconds after the stroke ends so the drawing can settle. It is enabled by default and can be toggled from the main menu; the preference is stored in browser storage.
